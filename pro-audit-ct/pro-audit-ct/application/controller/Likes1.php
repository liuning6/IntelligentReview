...
class Likes1 extends Common{
	...
	public function index(){
		...
		$where[] = ['exp', db()->raw('id in'. db('likes_210629')->field('pid')->where($map)->buildsql() . ' or id in' . db('likes_210629')->field('pid2')->where($map)->buildsql())];
		...
		foreach($data as $k=>$v){
			...
			$pids = [];
			$pid1 = db('likes_210629')->field("id, pid2 ppid, matching")->where($map)->where("pid={$v['id']}")->select();
			$pid2 = db('likes_210629')->field("id, pid ppid, matching")->where($map)->where("pid2={$v['id']}")->select();
	...
	public function pc(){
		...
		$ypid = $pid;
		if($type == 1){
			db('likes_210629')->where("pid = {$pid} or pid2 = {$pid}")->setField('x', 0);
			db('orders')->where("id = (select oid from photos where id = {$pid})")->setDec('cs');
		}else{
			db('likes_210629')->where('id', $pid)->setField('x', 0);
			$pid = db('likes_210629')->where('id', $pid)->value('pid');
			if(db('likes_210629')->where("pid = {$pid} and x = 1")->count() <= 0) db('orders')->where("id = (select oid from photos where id = {$pid})")->setDec('cs');
		}
